...
class admin extends Component {
    ...
    //Load metamask 
    async loadWeb3(){
        if(window.ethereum){ //If metamask API exist in the current browser
            window.web3 = new Web3(window.ethereum)
            await window.ethereum.request({method: 'eth_requestAccounts'})
        } else if(window.web3){ //If old metamask API version exists in the current browser
    ...
    registerUser = async(newUser) => {
        try {           
            await this.state.contract.methods.registerUser(newUser).send({from: this.state.currentAccount})
        } catch(err){
            console.log(err)
        }
    }
    ...
    ownerSignsDocument = async() => {
        try {
            const currentSignature = await this.signContract()
            console.log("Sign: " + currentSignature)
            
            await this.state.contract.methods.ownerSignsDocument(this.state.fileHash, currentSignature).send({from: this.state.currentAccount})
    ...
    //Owner signs to rescind the contract
    ownerRescindDocument = async() => {
        try {
            await this.state.contract.methods.ownerRescindDocument(this.state.fileHash).send({from: this.state.currentAccount})
        } catch(err){
            console.log(err)
        }
    }

    //Apply the contract revokation
    revokeDocument = async(userAddress) => {
        try {
            await this.state.contract.methods.revokeDocument(this.state.fileHash, userAddress).send({from: this.state.currentAccount})
        } catch(err) {
            console.log(err)
        }
    }
                ...
                <div className="container-fluid mt-5">
                    <div className="row">
                        ...
                        <div className='w-1/2 flex flex-col pb-12'>
                            <form onSubmit={(event) => {
                                event.preventDefault()
                                const userAddress = this.usrAddr.value
                                this.revokeDocument(userAddress)
                            }}>

                            <input type="textarea"
                                   className="form-control mb-1"
                                   placeholder="user address"
                                   ref={(input) => this.usrAddr = input} />        

                                <input type="submit"
                                    className='bbtn btn-block btn-warning btn-sm'
                                    value="Revoke document"/>
                            </form>
                        </div>
                        <br></br> <br></br>
                        <div className='w-1/2 flex flex-col pb-12'>
